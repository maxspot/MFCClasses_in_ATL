...
//			- The full MFC CStdString set of functions (including implicit cast)
...
//                  Arnt also noticed that CStdString was silently correcting the
...
//					make it as forgiving as CStdString's version is.  Thanks to
...
//				  - Thanks to Jason Mills for catching a case where CStdString was
...
//					range check as CStdString's does.  Now fixed -- thanks!
//
...
//
//	  2000-JUL-11 - Joe Woodbury noted that the CStdString::Find docs don't match
//					what the CStdString::Find code really ends up doing.   I was
//					trying to match the docs.  Now I match the CStdString code
...
//				  - Improved ReleaseBuffer() to be as forgiving as CStdString.
...
//
//	  1999-JUL-01 - Added a true CStdString facade.  Now you can use CStdString as
//					a drop-in replacement for CStdString.  If you find this useful,
//					you can thank Chris Sells for finally convincing me to give
//					in and implement it.
//				  - Changed operators << and >> (for MFC CArchive) to serialize
//					EXACTLY as CStdString's do.  So now you can send a CStdString out
...
//				  - Added equivalents of CStdString::GetBuffer, GetBufferSetLength
...
//      which take unsigned characters.  CStdString has such functions and in order
//      to provide maximum CStdString-compatability, this code needs them as well.
...
// ---------------------
//      This macro provides limited compatability with a questionable CStdString
//      "feature".  You can define it in order to avoid a common problem that
//      people encounter when switching from CStdString to CStdString.
//
//      To illustrate the problem -- With CStdString, you can do this:
//
...
//      is not.  Of course, neither is the type CStdString, but CStdString lets you do
...
//      const CT*) that CStdString (and MFC's CStdString) provide.  That was the
...
// -----------------------------------------------------------------------------
// ssfmtmsg: FormatMessage equivalents.  Needed because I added a CStdString facade
...
//
// REMARKS:
//		This template derives from basic_string<CT> and adds some MFC CStdString-
//		like functionality
//
//		Basically, this is my attempt to make Standard C++ library strings as
//		easy to use as the MFC CStdString class.
//
...
//		}
	...
	bool Load(UINT nId, HMODULE hModule=NULL)
	{
		...
		// land, you ought to be using CStdString for resources anyway since
	...
	#else // otherwise make our own hackneyed version of CStdString's Load
	...
	//		objects.  It looks a lot like MFC's CStdString::Format.  Some people
    ...
    // Answer:  One reason only - CStdString compatability.  In short, by making
	...
	// -------------------------------------------------------------------------
	// CStdString Facade Functions:
	//
	// The following methods are intended to allow you to use this class as a
	// near drop-in replacement for CStdString.
	// -------------------------------------------------------------------------
	#ifdef SS_WIN32
	...
	{
		// CStdString::Find docs say add 1 to nStart when it's not zero
		// CStdString::Find code doesn't do that however.  We'll stick
	...
	{
		// CStdString does range checking here.  Since we're trying to emulate it,
		// we must check too.
	...
	// ReverseFind overload that's not in CStdString but might be useful
	...
	// CStdString's OemToAnsi and AnsiToOem functions are available only in
	...
	// I have intentionally not implemented the following CStdString
...
// Note that we use an intermediate CStdString object in order to ensure that
// we use the exact same format.

#ifdef _MFC_VER
	inline CArchive& AFXAPI operator<<(CArchive& ar, const CStdStringA& strA)
	{
		CStdString strTemp(strA);
		return ar << strTemp;
	}
